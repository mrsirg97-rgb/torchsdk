...
**SDK Version:** 2.0.0
**On-Chain Program:** `8hbUkonssSEEtkqzwM7ZcZrD9evacM92TcWSooVF4BeT` (V3.0.0)
**Language:** TypeScript
**Test Result:** 22 passed, 0 failed (Surfpool mainnet fork)
...
This audit covers the Torch SDK v2.0.0, a TypeScript library that reads on-chain state from Solana and builds unsigned transactions for the Torch Market protocol. The SDK was cross-referenced against the live on-chain program (V3.0.0) to verify PDA derivation, quote math, vault integration, and account handling.
...
| File | Lines | Role |
|------|-------|------|
| `src/index.ts` | 89 | Public API surface (25 functions, ~30 types, 4 constants) |
| `src/types.ts` | 318 | All TypeScript interfaces |
...
| `src/torch_market.json` | — | Anchor IDL (V3.0.0) |
| **Total** | **2,884** | |
...
5. **E2E validation** via Surfpool mainnet fork (22/22 tests passed)
...
The Torch SDK v2.0.0 is a well-structured, minimal-surface TypeScript library that correctly mirrors the on-chain Torch Market V3.0.0 program. Key findings:
...
6. **No critical or high findings** — 3 low and 5 informational issues identified.
...
This audit was performed by Claude Opus 4.6 (Anthropic) on February 10, 2026. All source files were read in full and cross-referenced against the on-chain program. The E2E test suite (22/22 passed) validates the SDK against a Surfpool mainnet fork of the live program.

**Auditor:** Claude Opus 4.6
**Date:** 2026-02-10
**SDK Version:** 2.0.0
**On-Chain Version:** V3.0.0 (Program ID: `8hbUkonssSEEtkqzwM7ZcZrD9evacM92TcWSooVF4BeT`)
